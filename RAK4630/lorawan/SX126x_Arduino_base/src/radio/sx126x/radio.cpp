...
*/
	...
	void RadioSend(uint8_t *buffer, uint8_t size)
	{
		...
		// Serial.println("=====================RadioSend==========before=====SX126xSetPacketParams======");
		SX126xSetPacketParams(&SX126x.PacketParams);
		// Serial.println("=====================RadioSend==========after=====SX126xSetPacketParams======");
		// Serial.println("========BBB=============RadioSend=======before=======SX126xSendPayload=======");
		SX126xSendPayload(buffer, size, 0);
		// Serial.println("========BBB=============RadioSend=======after=======SX126xSendPayload=======");
	...
	void RadioRx(uint32_t timeout)
	{
		...
		if (RxContinuous == true)
		{
		Serial.println("sx1262 RadioRx RxContinuous");
	...
	void RadioIrqProcess(void)
	{
		if (IrqFired == true)
		{
			...
			{
				Serial.println("sx1262 IRQ_TX_DONE ");
			...
			{
				Serial.println("sx1262 IRQ_RX_DONE ");
				uint8_t size;
			...
			{
				Serial.println("sx1262 IRQ_RX_TX_TIMEOUT");
			...
			{
				Serial.println("sx1262 IRQ_PREAMBLE_DETECTED");
			...
			{
				Serial.println("sx1262 IRQ_HEADER_ERROR");
				TimerStop(&RxTimeoutTimer);
				if (RxContinuous == false)
				{
					//!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
		...
		if (TimerRxTimeout)
		{
			Serial.println("sx1262 TimerRxTimeout");
		...
		if (TimerTxTimeout)
		{
			Serial.println("sx1262 TimerTxTimeout");
